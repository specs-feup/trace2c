...
ctype knn(ftype xFeatures[NUM_FEATURES], ftype knownFeatures[NUM_KNOWN_POINTS][NUM_FEATURES], ctype knownClasses[NUM_KNOWN_POINTS])
{
    ...
    {
        n_xFeatures[i]++;
        fprintf(log_file_0, "\"xFeatures[%d]_%d_l\" [label=\"xFeatures[%d]\", att1=var, att2=param, att3=double ];\n", i, n_xFeatures[i], i);
    }
    for (int i = 0; i < NUM_KNOWN_POINTS; i++)
    {
        for (int j = 0; j < NUM_FEATURES; j++)
        {
            ...
            fprintf(log_file_0, "\"knownFeatures[%d][%d]_%d_l\" [label=\"knownFeatures[%d][%d]\", att1=var, att2=param, att3=double ];\n", i, j, n_knownFeatures[i][j], i, j);
        }
    }
    for (int i = 0; i < NUM_KNOWN_POINTS; i++)
    {
